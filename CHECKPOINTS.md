...
**Site rename "Voice Assistant" → "Voice Assist" — 2026-08-03.** User requested via uploaded `needtodo.txt`. Clarified scope first (AskUserQuestion: website branding only, vs. everywhere including body copy referring to the app itself) since "Voice Assistant" had been used interchangeably as both the site's own brand and the name of the promoted app — user chose "rename everywhere." Applied across: `messages/en.json` (10 strings — site name, under-construction notice, home/about/app/news hero copy, privacy policy body), `messages/bn.json` (9 strings, "অ্যাসিস্ট্যান্ট" → "অ্যাসিস্ট", including the possessive form), root layout metadata fallback, the OG image generator (both locales), `seo.ts`'s Organization/NewsArticle JSON-LD (`name`/`publisher.name`/`author.name`), the email adapter's `defaultFromName`, the contact-notification email subject line, the App Releases collection's admin description, and the internal (unlinked) `/design` demo page. `payload-types.ts` intentionally left untouched — it's auto-generated and self-corrects on next typegen. Verified: `tsc --noEmit` clean, a full repo grep for "Voice Assistant" outside `payload-types.ts`/`node_modules`/`.next` returns zero matches, and a live dev-server check of `/en` and `/bn` across home/about/app/news confirms "Voice Assist" (and "ভয়েস অ্যাসিস্ট") renders correctly everywhere with zero stray "Assistant" occurrences in the rendered HTML.

**Stats "+" suffix admin clarity fix — 2026-08-03.** Same `needtodo.txt` request: the admin was typing "+" into the wrong field when entering a stat like "100+ devices delivered." Investigated `StatTile.tsx` and the homepage's stats rendering first — the suffix auto-append (`{display}{suffix}{label}`) was already correct in code; there was no logic bug. The actual cause was unclear admin-form guidance, so fixed it at the source: `globals/HomePage.ts`'s `stats` array now has explicit `admin.description` + `admin.placeholder` on all three sub-fields (Value: "just the number, e.g. 100 — no + here", Suffix: "e.g. + turns 100 into 100+ automatically", Label: "no + here either"), plus a top-level array description reinforcing the same point. No frontend or data-model changes needed.

**Custom cursor (visual polish, not a checkpoint item) — 2026-08-04.** User asked for a "premium" hover effect like they'd seen on other sites — a circle instead of the plain mouse pointer. Confirmed style (dot + trailing ring, ring expands on hover) and scope (public site, desktop/mouse only) via AskUserQuestion before building. New `components/CustomCursor.tsx`: a small dot tracks the pointer exactly, a larger ring eases toward it, and the ring expands + tints over links/buttons/form controls. Deliberately skipped entirely — no native cursor hidden, no listeners attached — on touch devices (`pointer: fine` check) and when `prefers-reduced-motion` is set, rather than degraded in place; text inputs/textareas keep the native I-beam so typing affordance isn't lost. Uses the `--color-mist-green` design token so it automatically matches the brand accent in both light and dark themes. Verified with Playwright: simulated mouse movement over the homepage confirms the dot+ring render and track correctly, the ring visibly expands over the "Request a device" button in both light and dark mode, and an iPhone-13-emulated page never gets the `custom-cursor-active` class (confirming touch devices are unaffected). `tsc --noEmit` clean.

**Custom cursor fix — pointer detection on hybrid touchscreen laptops — 2026-08-04.** User reported the cursor never showed on their machine even after a clean rebuild; a diagnostic snippet run in their own DevTools console showed `pointerIsFine: false` on a normal laptop with a mouse/trackpad — `(pointer: fine)` reflects only the *primary* input, which some Windows hybrid touchscreen devices misreport as coarse. Switched the check to `(any-pointer: fine)` (is a fine pointer available at all) and added a one-time `touchstart` listener that instantly tears the whole effect down — removes the active class, hides dot/ring, unbinds every mouse listener — the moment a real touch happens, so a genuinely touch-primary device never gets stuck half-on. Separately confirmed with the user that `reducedMotionOn: true` (Windows' Animation-effects setting) was independently keeping the cursor off on their machine as designed — they chose to keep respecting that setting rather than override it. Delivered this fix as a single file write directly to their disk (not a zip) per their request to skip the extract/robocopy step for small changes going forward.

**Site-wide micro-animations — 2026-08-04.** User asked for "small micro animations to all pages." Surveyed existing motion first (the `Reveal` scroll-fade-in system already covers every page, plus `StatTile` count-up and a couple of hover states) so new work wouldn't duplicate it. Confirmed scope via AskUserQuestion: button/link press feedback, card + image hover polish, icon/toggle detail animations, and page-to-page transitions, all kept very subtle. Added: `active:scale-[0.97]` press feedback on `Button`/`ButtonLink`/`ButtonAnchor` and on Header's nav links/hamburger; the mobile nav panel now fades+drops in on open (`.menu-drop`) and the hamburger icon rotates into an X; `ThemeToggle`'s sun/moon icon pops in on swap (`.icon-pop`) and `LanguageToggle`'s active-locale pill does the same (`.pill-pop`); news/gallery listing card cover images zoom slightly on hover (matching the existing `AlbumCoverMosaic` treatment); gallery cards — which are fully link-wrapped — additionally get a whole-card lift (`.card-hover-lift`), deliberately *not* applied to news cards (only inner elements are links there) or to any static/non-clickable `Card` usage elsewhere, so nothing looks clickable that isn't; "Read more"/"back" links now nudge their arrow in the direction of travel on hover; `ContactForm` inputs get a soft focus ring, its buttons get press feedback, and the success panel pops in. Added a new `template.tsx` (not `layout.tsx` — templates remount on every navigation, layouts persist) so each page fades in on route change (`.page-fade-in`) without touching the persistent Header/Footer. Every new animation/transition falls under the site's existing blanket `prefers-reduced-motion` override — verified via Playwright that `animation-duration` collapses to ~0.01ms under reduced motion, same as the rest of the site. Also verified with Playwright screenshots (news/gallery hover states, mobile menu open, theme toggle) and confirmed zero console/page errors across all public routes in both locales. `tsc --noEmit` clean.

**Achievements & Our Journey pages added to nav — 2026-08-04.** User asked for a proper "Achievement" section and a "History" section showing how the project reached this point. Scope confirmed via AskUserQuestion: two dedicated full pages, both added to the nav (not folded into About); the history section named "Our Journey" (which happened to match copy already on the About page's existing milestone-timeline teaser — "Our journey" / "How we got here" — confirming the name fit); Achievements covering key numbers/milestones plus awards & recognition. Discovered the CMS already had most of the needed infrastructure sitting unused-for-this-purpose: a `Milestones` collection (research/release/outreach/award types) already powering a 10-item teaser timeline on the About page via the existing `TimelineItem` component, and a `Partners` collection already shown on the homepage. Built on top of that rather than duplicating it: added a new `Achievements` collection for "by the numbers" cards (same value/suffix/label shape as `HomePage.stats`, reusing `StatTile`, plus a description field for more context than a homepage counter allows); added an optional `awardingBody` field to `Milestones` (admin.condition, shown only when Type = Award) so award-type milestones can carry who gave the award; and pulled `Milestones` where `type: 'award'` into the new Achievements page's "Awards & Recognition" section — an award is both a dated Journey entry and a spotlight-worthy achievement, so it isn't duplicated into a second collection. New `/journey` page shows the full, unlimited Milestones timeline (oldest first); the About page's existing timeline section was trimmed from 10 items to a 4-item teaser with a new "See our full journey →" link to `/journey`, so the same content isn't fully duplicated across two pages. Both pages added to Header nav, Footer nav, and `sitemap.ts`, in both locales. Adding 2 nav items (8 total) caused real wrapping at the existing `lg` (1024px) breakpoint and even at 1440px, because the header's content width is capped by `max-w-6xl` regardless of viewport — caught via Playwright screenshots at 1024/1152/1280/1440/1920px, not assumed. Fixed by shortening "Our Journey" to "Journey" in the nav specifically (full name kept as the page's own hero title), tightening pill padding/gaps, and moving the desktop-nav breakpoint from `lg` to `xl` (1280px) so anything narrower correctly falls back to the already-existing mobile hamburger menu instead of squeezing 8 pills into too little room. Ran `pnpm generate:types` to regenerate `payload-types.ts` for the new collection/field. As with every other content checkpoint, no achievement/award/milestone data was fabricated — only technical scaffolding and structural copy (hero text, section headings, empty states) were written; real entries need to be added via `/admin`. Verified: `tsc --noEmit` clean, every new and existing public route returns 200 in both locales, `/admin` renders without a config error (confirms the new collection/field registered correctly), and Playwright confirms both the About page's new teaser link and the full Journey timeline render correctly once given time past the `Reveal` scroll-animation's fail-safe delay (an early screenshot taken too soon looked broken — false alarm, re-verified with a longer wait and a full scroll-through).

**Gallery listing redesigned as an image-only bento grid — 2026-08-04.** User shared a reference screenshot (an architecture-firm-style gallery) and asked to match it: no text visible on the grid except a title over a blurred bottom strip, tiles of uneven width arranged rhythmically rather than a uniform grid, and a slight hover animation, with the full album (image + text) opening on click. Replaced the old per-album card (mosaic of up to 4 thumbnails, title, date, photo count, description excerpt) with a single full-bleed cover photo per tile and only the title, overlaid in a black-gradient + `backdrop-blur-sm` caption strip — matching the reference's frosted-glass look. Desktop (md+) uses a 12-column grid with a repeating 3-tile span pattern (4/5/3, then 5/4/3 — sums to 12 either way) so any number of albums keeps the same bento rhythm, with a fixed row height so images of any source aspect ratio crop consistently via `object-cover`; mobile falls back to a plain 2-up `aspect-square` grid since the proportional-span rhythm doesn't translate to a narrow screen. Hover: cover image scales to 110% inside an `overflow-hidden` tile. Date/photo-count/description are untouched on the album detail page (`/gallery/[slug]`) — they were only ever removed from the listing, per the request. Verified against the sandbox's existing test albums (leftover placeholder-color test images from earlier phases, not new fabricated content): bento spans, blur/gradient caption, hover zoom, and the mobile 2-up fallback all screenshot correctly in Playwright; `tsc --noEmit` clean; listing and all three existing detail pages return 200 in both locales.
